...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'quantity', 'client', 'status', 'created_by', 
            'created_at', 'photo', 'estimation_date'
        ]
        read_only_fields = ['created_by', 'created_at', 'estimation_date']

    def validate_client(self, value):
        if value.role != 'CLIENT':
            raise serializers.ValidationError("The client must have the 'CLIENT' role.")
        return value
    ...
    def create(self, validated_data):
        ...
        latest_product = Product.objects.filter(status__in=['pending', 'doing']).order_by('-estimation_date').first()
